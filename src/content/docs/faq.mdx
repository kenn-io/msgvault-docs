...
Yes. You can sync from any standard IMAP server using `msgvault add-imap`, import email from local files using `import-mbox` (standard MBOX archives) or `import-emlx` (Apple Mail exports), and import text messages from WhatsApp, iMessage, and Google Voice. All messages are stored in the same database and work with all the same tools: search, TUI, MCP server, web server, and export. See [Setup Guide](/setup/#add-an-imap-account) for IMAP setup, [Importing Local Email](/usage/importing/) for file imports, and [Text Messages](/usage/text-messages/) for chat imports.

<p class="faq-question">Does deleting email in msgvault delete it from Gmail?</p>

Only if you explicitly run the full deletion workflow. Staging messages for deletion in the TUI does not touch Gmail or your IMAP provider. You must run `msgvault delete-staged` to permanently delete staged messages from Gmail or your IMAP server. This is irreversible, so review your staged manifests carefully. Your local archive is always preserved. See [Deleting Email](/usage/deletion/) for the complete process.
