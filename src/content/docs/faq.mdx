...
Yes. You can sync from any standard IMAP server using `msgvault add-imap`, or import email from local files using `import-mbox` (standard MBOX archives) or `import-emlx` (Apple Mail exports). All messages are stored in the same database and work with all the same tools: search, TUI, MCP server, web server, and export. See [Setup Guide](/setup/#add-an-imap-account) for IMAP setup and [Importing Local Email](/usage/importing/) for file imports.
