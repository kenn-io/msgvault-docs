...
HTTP 400 during `msgvault build-embeddings`.
...
If `msgvault build-embeddings` logs `HTTP 400`, msgvault now includes
...
not depend on the vector index. Once `msgvault build-embeddings`
...
| `index_stale` | Active generation's fingerprint does not match the current model, dimension, preprocessing policy, `max_input_chars`, or embedding output policy. | Run `msgvault build-embeddings --full-rebuild --yes`. |
