...
it wires everything up automatically. To invoke `go build` yourself
from PowerShell:
...
If `msgvault embed` fails with repeated `HTTP 400` warnings, check
...
<Aside type="tip">
You can interrupt and resume. Each invocation of `msgvault embed`
processes whatever is currently pending and activates the generation
when the queue reaches zero. `Ctrl+C` is safe; run `msgvault embed`
...
active generation during the sync. Run `msgvault embed` (no
...
`msgvault embed` without `--full-rebuild` is a short, incremental
...
every sync, or chain it (`sync-incremental && embed`).
...
`msgvault embed --full-rebuild --yes` rebuilds the index over the
...
does not depend on the vector index. Once `msgvault embed` reports
...
`msgvault embed` repeatedly logs `embed batch failed ... HTTP 400`
