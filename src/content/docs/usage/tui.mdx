...
Press `m` to toggle between Email and Texts mode. This mode is only available when text data has been imported. See [Text Messages](/usage/text-messages/) for details on importing WhatsApp, iMessage, and Google Voice conversations.
