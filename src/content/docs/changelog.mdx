...
- Advanced search: word-boundary regex matching, improved FTS5 prefix search, and snippets in fast search results.
- Expanded store API with structured query parsing for search (`SearchMessagesQuery`).

**Improvements**

- Search result quality: snippets now surface in aggregate/fast-path queries. SQLite aggregate sort ties are broken deterministically by key.
