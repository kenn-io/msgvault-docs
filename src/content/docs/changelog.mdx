...
- Advanced search: word-boundary regex matching, improved FTS5 prefix search, snippets in fast search results, and deterministic sort ordering.
- Expanded store API with structured query parsing for search and direct message access.

**Improvements**

- Search result quality: snippets now surface in aggregate/fast-path queries, and sort ties are broken deterministically by key.
...
- `import-mbox` accepts files with any extension (not just `.mbox`/`.mbx`), `--label` is repeatable/comma-separated, and re-imports update labels on existing messages instead of silently skipping.
