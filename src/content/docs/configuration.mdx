...
External OpenAI-compatible embedding endpoint used to convert message text into vectors. msgvault does not host a model, it calls out to one you run (Ollama, llama.cpp `server`, LM Studio, a hosted OpenAI endpoint, etc.).

| Key | Default | Description |
|---|---|---|
| `endpoint` | (required) | HTTP(S) URL of an OpenAI-compatible `/embeddings` endpoint. |
