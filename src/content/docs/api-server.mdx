...
replaces `messages`. `pool_saturated` is true when the number of
results equals the per-signal candidate pool, hinting that some
relevant hits may have been cut off.
